-- Chat System Database Updates Only
-- This file only updates the chat system without modifying existing tables

-- 1. Update the existing chats table with new columns
...
-- 2. Create a new table for chat participants/rooms
...
-- 4. Create a function to automatically update chat_rooms when new messages are inserted
...
-- 5. Create trigger to automatically update chat rooms
...
-- 8. Create a view for active chat rooms with participant info
...
-- 9. Enable Row Level Security (RLS) for better security
ALTER TABLE public.chat_rooms ENABLE ROW LEVEL SECURITY;

-- 10. Create RLS policies for chat_rooms
...
-- 11. Create RLS policies for chats (if not already enabled)
...
-- 13. Create a function to get unread message count
...
-- 14. Add missing columns to user_profiles if they don't exist
ALTER TABLE public.user_profiles 
ADD COLUMN IF NOT EXISTS avatar_url TEXT,
ADD COLUMN IF NOT EXISTS points INTEGER DEFAULT 0;

-- 15. Create indexes for user_profiles
CREATE INDEX IF NOT EXISTS idx_user_profiles_points ON public.user_profiles(points DESC);

-- 16. Add constraints for points
...
-- 17. Create a function to award points
