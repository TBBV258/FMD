...
-- 6. Trigger 1: Notificação de nova mensagem de chat
...
-- 7. Trigger 2: Notificação de mudança de status de documento
...
-- 8. Trigger 3: Notificação de verificação de documento
...
-- 9. Trigger 4: Notificação de marcos de pontos
...
WHERE trigger_schema = 'public'
  AND event_object_table IN ('chats', 'documents', 'user_profiles', 'verification_requests')
ORDER BY event_object_table, trigger_name;

-- 11. Row-Level Security (RLS) - Permitir usuários atualizarem seus próprios perfis
DROP POLICY IF EXISTS "Users can update own profile" ON public.user_profiles;
CREATE POLICY "Users can update own profile" ON public.user_profiles
FOR UPDATE USING (auth.uid() = id);

-- 12. Permitir usuários lerem notificações próprias
...
FROM information_schema.columns 
WHERE table_schema = 'public' 
  AND table_name IN ('user_profiles', 'documents')
  AND column_name IN ('delivery_address', 'rank', 'location_metadata')
...
-- ============================================
-- SUCESSO!
-- Se chegou aqui sem erros, a migração está completa.
-- ============================================
